...
function getOpenAiOutputText(payload: any) {
  ...
  const textPart = content.find((part: any) => part?.type === 'output_text' || part?.type === 'text');
...
async function callHuggingFaceSegmentation(imageUrl: string) {
  ...
  if (!imageResponse.ok) {
    throw new Error(`Unable to fetch wardrobe image for Hugging Face: ${imageResponse.status}`);
  }
  ...
  if (!segmentationResponse.ok) {
    ...
    throw new Error(`Hugging Face segmentation failed: ${segmentationResponse.status} ${errorText}`);
  }
...
function wardrobeAnalysisSchema() {
  return {
    ...
    properties: {
      ...
      sleeve: { type: ['string', 'null'], enum: ['sleeveless', 'short', 'long', null] },
      colours: {
        type: 'array',
        items: { type: 'string' },
      },
      pattern: { type: 'string', enum: ['solid', 'stripes', 'checks', 'textured'] },
      seasons: {
        type: 'array',
        items: { type: 'string', enum: ['summer', 'winter', 'monsoon', 'all-season'] },
      },
      occasions: {
        type: 'array',
        items: {
          type: 'string',
          enum: ['Office', 'Party', 'Date', 'Wedding', 'Casual', 'Gym', 'Travel'],
        },
      },
      formality: {
        type: 'string',
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (body.action === 'analyze-item') {
      if (!body.imageUrl) {
        return jsonResponse(400, { error: 'imageUrl is required for analyze-item.' });
      }
    ...
    return jsonResponse(500, {
      error: error instanceof Error ? error.message : 'Unknown AI integration error.',
    });
  }
});
