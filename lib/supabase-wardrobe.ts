...
function mapWardrobeRow(row: WardrobeItemRow): WardrobeItem {
  return {
    id: row.id,
    name: row.name,
...
function mapWardrobeInsert(item: WardrobeItem, imageUrl?: string, imageStoragePath?: string): WardrobeItemInsert {
  return {
    name: item.name,
    ...
    ai_tags: item.aiTags ? (item.aiTags as unknown as Record<string, unknown>) : null,
  };
}

export async function listWardrobeItemsFromSupabase() {
  ...
  const hydratedRows = await Promise.all(
    rows.map(async (item) => {
      ...
      const signedUrlResult = await client.storage
        ...
        .createSignedUrl(item.imageStoragePath, 60 * 60);
...
export async function uploadWardrobeItemToSupabase(item: WardrobeItem, asset: ImagePickerAsset) {
  const client = supabase;
  ...
  if (asset.uri) {
    ...
    imageStoragePath = `wardrobe/${Date.now()}-${sanitizeFilename(asset.fileName ?? `upload.${extension}`)}`;
  ...
  const payload = mapWardrobeInsert(item, undefined, imageStoragePath);
  const { data, error } = await client
    .from('wardrobe_items')
    .insert(payload)
    .select('*')
...
export async function updateWardrobeItemInSupabase(item: WardrobeItem) {
  ...
  const signedUrlResult = await client.storage
    .from(wardrobeBucketName)
    .createSignedUrl(mapped.imageStoragePath, 60 * 60);
