...
export type StylePreference = 'casual' | 'formal' | 'street' | 'ethnic' | 'minimal';
...
export type Formality = 'casual' | 'smart' | 'formal' | 'festive' | 'athleisure';
...
function getTemperatureScore(item: WardrobeItem, weather: WeatherSnapshot) {
  ...
  if (weather.condition === 'Rain' && item.category === 'Shoes' && item.colours.includes('white')) {
    return -10;
  }
...
function getProfileScore(item: WardrobeItem, profile: UserProfile) {
  ...
  if (profile.occasionPreference === 'office-heavy' && item.occasions.includes('Office')) {
    score += 4;
  }
...
function getColourHarmonyScore(items: WardrobeItem[]) {
  ...
  for (let index = 0; index < colours.length - 1; index += 1) {
    ...
    if (colourPartners[current]?.includes(next) || colourPartners[next]?.includes(current)) {
      score += 6;
      continue;
    }
  ...
  const accessory = getItemsByCategory(items, 'Accessory').find(
    (item) =>
      ...
      (neutralColours.has(item.colours[0]) ||
        outfit.some((outfitItem) => colourPartners[outfitItem.colours[0]]?.includes(item.colours[0])))
  );
...
function buildReasons(
  ...
  if (weather.temperatureC >= 29) {
    reasons.push('Lightweight fabrics keep the look breathable for a warm day.');
  }
  ...
  if (occasion === 'Office') {
    reasons.push('The outfit balances polished pieces with relaxed structure for work.');
  }

  if (occasion === 'Travel') {
    reasons.push('Comfort-first pieces make the outfit easy to repeat across transit and city time.');
  }

  if (outfit.some((item) => item.lastWornDaysAgo >= 10)) {
    reasons.push('It rotates in an underused piece so your closet gets more mileage.');
  }

  if (accessory) {
    reasons.push(`${accessory.name} completes the outfit without adding styling friction.`);
  }
...
export function buildRecommendations(
  ...
  for (const top of tops) {
    for (const bottom of bottoms) {
      for (const shoe of shoes) {
        ...
        if (context.weather.temperatureC <= 22 || context.weather.condition === 'Rain') {
        ...
        const confidence = Math.max(68, Math.min(97, 68 + Math.round(totalScore / 6)));
  ...
  return suggestions.sort((first, second) => second.score - first.score).slice(0, 5);
}
...
export function buildGapInsights(items: WardrobeItem[]) {
  ...
  const beigeBottoms = bottoms.filter((item) => item.colours.includes('beige')).length;
  ...
  if (blueTops >= 1 && beigeBottoms === 0) {
    insights.push({
      ...
      detail: 'You have blue tops in rotation, but no beige bottom that broadens summer office pairings.',
    });
  }
  ...
  if (formalShoes.length < 2) {
    insights.push({
      ...
      detail: 'A second polished shoe option would reduce repetition for office and event looks.',
    });
  }
...
export function buildWeeklyPlan(
  ...
  const contexts: WeeklyPlanEntry['context'][] = [
    {
      occasion: 'Office',
      weather: { ...baseWeather, dayPart: 'Morning', temperatureC: 29, condition: 'Sunny' },
    },
    {
      occasion: 'Office',
      weather: { ...baseWeather, dayPart: 'Morning', temperatureC: 31, condition: 'Sunny' },
    },
    {
      occasion: 'Casual',
      weather: { ...baseWeather, dayPart: 'Afternoon', temperatureC: 30, condition: 'Cloudy' },
    },
    {
      occasion: 'Office',
      weather: { ...baseWeather, dayPart: 'Morning', temperatureC: 28, condition: 'Rain' },
    },
    {
      occasion: 'Party',
      weather: { ...baseWeather, dayPart: 'Evening', temperatureC: 26, condition: 'Cloudy' },
    },
    {
      occasion: 'Date',
      weather: { ...baseWeather, dayPart: 'Evening', temperatureC: 27, condition: 'Sunny' },
    },
    {
      occasion: 'Travel',
      weather: { ...baseWeather, dayPart: 'Morning', temperatureC: 29, condition: 'Cloudy' },
    },
  ];
...
export function buildPackingList(plan: WeeklyPlanEntry[]) {
  const selectedDays = plan.filter(
    (entry, index) => entry.context.occasion === 'Travel' || index >= plan.length - 3
  );
...
export function filterWardrobe(items: WardrobeItem[], category: Category | 'All', query: string) {
...
export function getWardrobeStats(items: WardrobeItem[]) {
  ...
  const officeReady = items.filter((item) => item.occasions.includes('Office')).length;
